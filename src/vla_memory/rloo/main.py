"""RLOO entry point — runs inside the micromamba `robomme` env.

Launched by modal_train.py::rloo via:
    micromamba run -n robomme python -u src/vla_memory/rloo/main.py --port N ...
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RLOO main loop (micromamba env)")
    ...
    parser.add_argument("--dynamic-sampling", action=argparse.BooleanOptionalAction, default=False,
                        help="RLOO has a valid baseline for every state; off by default.")
    ...
    print(
        "[rloo] config: " + " ".join(f"{k}={v}" for k, v in sorted(vars(args).items())),
        flush=True,
    )
    ...
    from vla_memory.rloo.trainer import RLOOConfig, RLOOTrainer
    ...
    # Resolve SFT adapter (same logic as grpo/main.py).
    ...
    print(f"RLOO loading SFT adapter from {resolved_adapter}", flush=True)
